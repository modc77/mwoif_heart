...
def parse_friend_list_response(raw: bytes, *, capacity: int = 300) -> FriendCapacity:
    ...
    for path, values in groups.items():
        ...
        if not count or count > capacity:
            continue
        # Repeated friend records should give the largest same-path id group.
        # Prefer deeper paths on a tie because a player id usually lives inside
        # an embedded Friend message rather than directly at response root.
